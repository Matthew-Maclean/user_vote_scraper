...
pub fn get_code() -> Result<String, CodeError> {
    let oauth_url =
        format!("https://www.reddit.\
                 com/api/v1/authorize?client_id={client_id}&response_type={response_type}&state={state}&redirect_uri={uri}&duration={duration}&scope={scope}",
...
pub fn get_token(code: &str, client: &hyper::Client) -> Result<String, TokenError> {
    ...
    let post_body =
        format!("grant_type={grant_type}&code={code}&redirect_uri={uri}",
    ...
    let header = {
        ...
        h.set(hyper::header::Authorization(hyper::header::Basic {
...
impl fmt::Display for CodeError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &CodeError::NoCodeInUrl => {
                write!(f, "The provided URL does not contain a 'code' parameter")
            }
            &CodeError::AccessDenied => write!(f, "The user did not allow this application access"),
            &CodeError::OtherAuthError => {
                write!(f, "Another error has occured (probably not your fault)")
            }
        }
    }
}
...
impl fmt::Display for TokenError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            &TokenError::HeaderError => {
                write!(f,
                       "There was an error with the POST header (not your fault)")
            }
