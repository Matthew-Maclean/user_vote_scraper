...
impl Comment {
    pub fn scrape(client: &hyper::Client,
                  token: &str,
                  user: &str,
                  limit: i32)
        ...
        let headers = {
            ...
            h.set(hyper::header::Authorization(hyper::header::Bearer { token: token.to_owned() }));
            h.set(hyper::header::UserAgent("windows: user_vote_scraper (pre-alpha)".to_owned()));
            h
        };

        let url_base = if limit > 100 {
            format!("https://oauth.reddit.com/user/{user}/comments/.json?limit=100",
                    user = user)
        } else {
            format!("https://oauth.reddit.com/user/{user}/comments/.json?limit={limit}",
                    user = user,
                    limit = limit)
        };
...
impl fmt::Display for Comment {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        write!(f,
               "https://www.reddit.com/comments/{link_id}/_/{id}",
               link_id = &self.link_id[3..self.link_id.len()],
               id = self.id)
    }
}
